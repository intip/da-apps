...
class Adm(object):
    ...
    def _getAllCar(self):
        """
            Retorna todas caracteristicas
        """
        for i in self.execSql("select_caracteristicas"):
            yield i
    
    @dbconnectionapp
    def _getCar(self, id_conteudo):
        """
            Retorna as caracteristicas de um signo
        """
        lista = []
        for i in self.execSql("select_cara_conteudo", id_conteudo=id_conteudo):
            lista.append(i["id_caracteristica"])
        return lista
