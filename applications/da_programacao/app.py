...
haslist = True
haslink = True
title = "DA-Programacao"
meta_type = "da_programacao"
...
class App(BaseApp, PortalUtils, Adm, Public):
    ...
    def _install(self, title,
        ...
        secaos=list(self.execSql("select_secaos"))
        id_secao=None
        nexval="select_nextval_secao"
        while id_secao < len(secaos):
            id_secao=self.execSql(nexval).next()["id"]

        tipos=list(self.execSql("select_tipos"))
        id_tipo=None
        nexval="select_nextval_tipo"
        while id_tipo < len(tipos):
            id_tipo=self.execSql(nexval).next()["id"]
    ...
    def retornarWidgets():
        """
            Método padrão que retorna a informação
            dos widgets que a aplicação utiliza
        """
        return ({"action": "viewd",
                ...
                {"action": "view",
                 "img": "/imgs/previewe.gif",
                 "titulo": "Programas vinculados",
                 "url": "listagemprogramasvinculados.env",
                 "target": "listagem"},
                {"action": "view",
                 "img": "/imgs/previewe.gif",
                 "titulo": "Programas",
                 "url": "listagemprogramas.env",
                 "target": "listagem"},)

    @dbconnectionapp
    def _verifyStatusContent(self, id_conteudo):
        """
            Método padrão interno de verificação
            do status de um conteúdo da aplicação
        """
    ...
    def _getListContentRef(self):
        """
            Método padrão para retornar as referências de
            um conteúdo a ser copiado em folders
        """
        for i in self.execSql("select_dados"):
    ...
    def copyFiles2Temp(self, id_pk, id_site=None):
        """
            Método padrão de cópia de arquivos do conteúdo
            do aplicativo para outras folders
        """
