...
class Public(object):
    ...
    """
    @dbconnectionapp
    def _get_consultoria(self, limit=10, offset=0):
        """
        	Retorna do banco os dados cadastrados em consultoria juridica
        """
        lista = []
        for i in self.execSql("select_consultoria_limit",
                              limit=int(limit),
                              offset=int(offset)):
            lista.append(i)
    ...
    @jsoncallback
    def get_consultoria(self, limit=10, offset=0):
        return self._get_consultoria(limit=limit, offset=offset)
