...
try:
    from xlwt import Workbook, XFStyle, Font, Alignment, Pattern
except ImportError:
    pass
        ...
        if int(id_relatorio) == 1 or int(id_relatorio) == 3:
            ...
            if int(id_relatorio) == 1:
                campos_s1 = [{"nome": "Nome"},
                             {"email": "email"},
                             {"cpf": "CPF"},
                             {"quantidade de participacoes": "Quantidade de participações"},
                             {"status":"status"},
                             {"bloqueio":"bloqueado"}]
                sheet.write_merge(0, 0, 0, 5, self.dec("Relação usuário participação por promoção"), style0)
            else:
                campos_s1 = [{"nome":"Nome"},
                             {"email":"email"},
                             {"cpf":"CPF"},
                             {"telefone":"Telefone"},
                             {"endereco":"Endereco"},
                             {"data_hora":"Hora participacao"}]
                sheet.write_merge(0, 0, 0, 5, self.dec("Dados de usuários por promoção"), style0)
            total_geral = 0
            count = 0               
            for i in promocoes:
                ...
                for j in range(len(campos_s1)):
                    sheet.write(index, j, campos_s1[j].values()[0].decode("utf-8"), style)
                ...
                if cont:
                    ...
                    total_geral += cont
                    index += 1
                    sheet.write(index, 0, self.dec('Total Participações'), style)
                    sheet.write(index,
                                1,
                                count_participacoes)
            ...
            index += 1
            sheet.write(index, 0, self.dec("Total de Participações Geral"), style)
            sheet.write(index, 1, self.dec(count), style)

        else:
            cont = 0
            sheet.write_merge(0, 0, 0, 5, self.dec("Relação de usuários contemplados"), style0)
            campos_s1 = [{"nome":"Nome"},
                         {"email":"email"},
                         {"cpf":"CPF"},
                         {"telefone":"Telefone"},
                         {"endereco":"Endereco"},
                         {"Contemplações":"Contemplações"}]
            index += 1
            for j in range(len(campos_s1)):
                    sheet.write(index, j, campos_s1[j].values()[0].decode("utf-8"), style)
                    sheet.col(j).width = 30 * 256         
            ...
            index += 6
            sheet.write(index, 0, self.dec("Total de usuários"), style)
