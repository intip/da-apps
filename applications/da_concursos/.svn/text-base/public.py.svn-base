...
class Public(object):
    ...
    @dbconnectionapp
    def _getFreeFiles(self, provas=False, limit=10, offset=0):
        """
            retorna ou provas
            cadastradas
        """
        if provas:
            res = [i for i in self.execSql("select_provas",
                                           limit=int(limit),
                                           offset=int(offset))]
        else:
            res = [i for i in self.execSql("select_files",
                                           limit=int(limit),
                                           offset=int(offset))]
        return res
    ...
    def getArquivos(self, provas=False, limit=10, offset=0):
        """
            retorna arquivos
            cadastrados callback
        """
        return self._getFreeFiles(provas, limit, offset)
